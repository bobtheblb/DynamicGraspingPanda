...
int main(int argc, char ** argv)
{
  ...
  auto const collision_object = [frame_id = move_group_interface.getPlanningFrame(), &node2] {
    ...
    collision_object.header.stamp = node2->now();
   
    // Assign a unique ID to the collision object
    collision_object.id = "conveyor_belt";
    ...
    primitive.dimensions[primitive.BOX_X] = 0.45;  // Width
    primitive.dimensions[primitive.BOX_Y] = 1.2;  // Depth
    primitive.dimensions[primitive.BOX_Z] = 0.73;  // Height
 
    // Define the pose (position and orientation) of the box
    geometry_msgs::msg::Pose box_pose;
   
    // Set the position of the box center
    box_pose.position.x = 0.55;  // meters in x-direction
    box_pose.position.y = 0;   // Centered in y-direction
    box_pose.position.z = 0.365;  // meters in z-direction
